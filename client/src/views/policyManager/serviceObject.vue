      ...
      <el-form ref="form" :model="searchForm" label-width="120px" label-padding-right="20px">
        <div class="blockbase block-rounded">
          <div class="block-header">
            <h3 class="block-title">서비스 객체</h3>
          </div>
          ...
          <div class="block-content">
            ...
            <el-table :data="pagedTableData" >
              ...
              <el-table-column label="포트">
                  <template slot-scope="scope">
                  <p v-for="(port, index) in scope.row.port" :key="`port-${index}`">{{port}}</p>
                  </template>
              </el-table-column>
            </el-table>
   ...
   components: {
    ...
    methods: {
      ...
      getDataLoad() {
        this.items = [];
        let data;
        console.log(this.searchForm.searchField);
        switch(this.searchForm.searchField){
          ...
          case "port":
            data = {
              device_id : this.searchForm.deviceId,
              port : this.searchForm.searchString,
            }
            break;
          default: 
            data = {
