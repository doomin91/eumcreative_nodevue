        ...
        <div class="blockbase block-rounded">
            ...
            <el-table :data="pagedTableData" >
                ...
                <el-table-column label="사유">
                    <template slot-scope="scope">
                    <span>
                        {{ getWordCount(scope.row.APPLY_TITLE, 30, "...") }}
                        <!-- <router-link :to="`/inbox/`+flag+'/'+scope.row.APPLY_SEQ" tag="div">{{ getWordCount(scope.row.APPLY_TITLE, 30, "...") }}</router-link> -->
                        <!-- <a :href="`/inbox/`+flag+'/'+scope.row.APPLY_SEQ">{{ getWordCount(scope.row.APPLY_TITLE, 30, "...") }}</a> -->
                    </span>
                    </template>
                </el-table-column>
                <el-table-column
                ...
                :filter-method="filterApplyStatus">
                    <template slot-scope="scope">
                        <el-tag size="mini" :type="getApplyStatusType(scope.row.APPLY_STATUS)">{{  getApplyStatus(scope.row.APPLY_STATUS) }}</el-tag>
                    </template>
                </el-table-column>
                <el-table-column 
                label="작업구분" 
                ...
                <el-table-column label="신청자" width="180">
                    <template slot-scope="scope">
                    <span>{{ scope.row.APPLY_USER_NAME }} {{ scope.row.APPLY_USER_POSITION }}</span>
                    </template>
                </el-table-column>
                <el-table-column label="신청부서" width="150">
                    <template slot-scope="scope">
    ...
    export default {
        ...
        methods:{
            ...
            goViewPage(seq){
                ...
                this.$router.push({path: "/inbox/"+this.flag+"/"+seq })
            }
        },
        components: {
            Breadcrumb
